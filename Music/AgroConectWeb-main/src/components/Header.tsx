...
import { getNegociacoes, getAlertas, markAlertaAsRead, Alerta } from "@/lib/firestoreService";
  ...
  useEffect(() => {
    if (currentUser) {
      Promise.all([
        ...
        const negsCount = negs.filter(n => 
          (n.proprietarioUid === currentUser.uid && n.status === 'pendente') ||
          (n.arrendatarioUid === currentUser.uid && (n.status === 'aceite' || n.status === 'recusada'))
        ).length;
        
        setAlertsList(alts);
        const unreadAlerts = alts.filter(a => !a.lido).length;
        setNotificationCount(negsCount + unreadAlerts);
      }).catch(console.error);
    } else {
          ...
          <div className="flex h-16 items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                variant="ghost"
                size="sm"
                className={`md:hidden h-9 w-9 p-0 rounded-lg relative ${
                  scrolled || !isHome ? "" : "text-white hover:bg-white/15"
                }`}
                onClick={() => setAlertsOpen(!alertsOpen)}
                aria-label="Notificações"
