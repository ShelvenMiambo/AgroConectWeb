...
import {
  getProperties, addProperty, deleteProperty, Property, createNegociacao,
  Listing, ListingType, getListings, addListing, deleteListing
...
/* ── Contact Modal ──────────────────────────────────── */
const ContactModal = ({
  property, onClose, onUpgrade
}: { property: Property; onClose: () => void; onUpgrade: () => void }) => {
  const { currentUser, userData } = useAuth();
        ...
        {userData?.plan === 'gratuito' && !IS_PROMOTION_FREE ? (
          <div className="text-center py-6 space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button className="flex-1 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold gap-2" onClick={() => { onClose(); onUpgrade(); }}>
...
const Marketplace = () => {
  ...
  const [upgradePlan, setUpgradePlan] = useState<'mensal' | 'trimestral' | 'anual' | undefined>(undefined);

  const load = async () => {
    setLoadingData(true);
    try {
      const [props, lsts] = await Promise.all([getProperties(), getListings()]);
      setProperties(props.items); setListings(lsts.items);
                  ...
                  <div className="relative overflow-hidden rounded-2xl bg-muted/30 border border-border/60 p-8 text-center mt-6">
                    ...
                    <div className="relative z-10 flex flex-col items-center max-w-md mx-auto">
                      ...
                      <div className="w-full space-y-3">
                         <Button onClick={() => { setUpgradePlan('mensal'); setShowUpgrade(true); }} className="w-full h-11 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold shadow-soft">Desbloquear Mensal — 200 MT</Button>
                         <Button onClick={() => { setUpgradePlan('trimestral'); setShowUpgrade(true); }} className="w-full h-11 rounded-xl bg-primary hover:bg-primary/90 text-white font-bold shadow-soft">Desbloquear Trimestral — 580 MT (Poupa 10%)</Button>
                         <Button onClick={() => { setUpgradePlan('anual'); setShowUpgrade(true); }} className="w-full h-11 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold shadow-soft">Desbloquear Anual — 2000 MT (Melhor Valor)</Button>
                      </div>
    ...
    <div className="min-h-screen bg-background">
      ...
      {contactProperty && (
        <ContactModal
          ...
          onUpgrade={() => { setContactProperty(null); setShowUpgrade(true); }}
        />
      )}
      {showUpgrade && (
        <UpgradeModal
          ...
          reason="para ver detalhes do terreno"
        />
      )}
